...
type MessageLike = {
  ...
  tool_calls?: Array<{ id?: string; name?: string; args?: unknown; function?: { name?: string; arguments?: unknown } }>;
};
...
function trendFromText(value: unknown): "positive" | "neutral" | "negative" {
  const text = typeof value === "string" ? value.toLowerCase() : "";
  if (text.includes("high") || text.includes("alert") || text.includes("moderate")) return "negative";
  if (text.includes("low") || text.includes("stable")) return "positive";
  return "neutral";
}

function cryptoBriefFromRenderA2uiArgs(rawArgs: unknown): CryptoBriefResponse | null {
  const args = parseToolArguments(rawArgs);
  if (!args || typeof args !== "object") return null;
  const components = (args as { components?: unknown }).components;
  ...
  const root = components.find((component) => {
    return Boolean(
      component &&
        ...
        (component as { id?: unknown }).id === "root",
    );
  }) as Record<string, unknown> | undefined;

  const rawChildren = (root?.props as { children?: unknown } | undefined)?.children;
  const children = Array.isArray(rawChildren) ? rawChildren : components;
  const blocks: CryptoUIBlockUnion[] = [];
  let title = "Interactive crypto analysis";
  ...
  for (const child of children) {
    ...
    const kind = item.type ?? item.component;
    const props = item.props ?? {};

    if ((kind === "text" || kind === "Text") && typeof props.text === "string") {
      title = props.text;
      continue;
    }

    if (kind === "summary_cards") {
      const label = textValue(props.label, "Risk score");
      const value = textValue(props.value, "Review needed");
      blocks.push({
        id: item.id ?? "a2ui-summary",
        type: "summary_cards",
        title: "A2UI generated summary",
        ...
        data: [
          {
            label,
            value,
            trend: trendFromText(value),
            description: "Generated by the interactive UI tool call.",
          },
        ],
      });
      continue;
    ...
    if (kind === "signal_list" && Array.isArray(props.items)) {
      blocks.push({
        id: item.id ?? "a2ui-signals",
        type: "signal_list",
        title: "A2UI generated signals",
        source: "llm_generated",
        isMockData: true,
        data: props.items
          ...
          .map((signal) => ({
            label: textValue(signal.title ?? signal.label, "Signal"),
            sentiment: trendFromText(signal.title ?? signal.description),
            description: textValue(signal.description, "Generated signal from the A2UI tool call."),
            relatedAssetIds: ["ada"],
            relatedProjectIds: [],
          })),
      });
  ...
  return {
    answer: `${title} was generated through the interactive UI path. The canvas below summarizes the generated risk view using the same tool call data.`,
...
function latestRenderA2uiBrief(messages: MessageLike[]): CryptoBriefResponse | null {
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    ...
    for (let j = calls.length - 1; j >= 0; j -= 1) {
      ...
      if (name !== "render_a2ui") continue;
      const brief = cryptoBriefFromRenderA2uiArgs(call.function?.arguments ?? call.args);
...
export function buildCryptoBriefFromMessages(messages: unknown): CryptoBriefResponse | null {
  ...
  const renderA2uiBrief = latestRenderA2uiBrief(typedMessages);
